...
- (NSString *)generatePOSTBodyFromDictionary:(NSDictionary *)dict {
    ...
    // DJA: There is an issue here with values containing literal &'s (like our artist and title strings)
    //[rawBody appendString:[NSString stringWithFormat:@"&%@=%@", key, [dict objectForKey:key]]];

    // What's completely unbelievable is that stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding
    // replaces spaces with %20 but doesn't even touch & or / or any other reserved URL character.

    // So have to do this C bullshit.
    NSString *encodedValue = (NSString *)CFURLCreateStringByAddingPercentEscapes(NULL, (CFStringRef)[dict objectForKey:key], NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
    [rawBody appendString:[NSString stringWithFormat:@"&%@=%@", key, encodedValue]];
